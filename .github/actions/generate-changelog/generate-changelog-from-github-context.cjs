  ...
  switch (gh.event_name) {
    ...
    case 'workflow_dispatch': {
      ...
      if (branchName.startsWith('pr/')) {
        ...
        lines.push('')
        const commits = getCommitsBetween(prDetails.base.sha, prDetails.head.sha)
        commits.forEach((commit) => {
          lines.push(` • [${commit.hash.slice(0, 8)}] ${commit.message}`)
        })
      ...
      lines.push(`GitHub manual build on "${branchName}", triggered by ${triggeredBy}`)
      if (notes) {
        lines.push(`Notes: ${notes}`)
      }
...
---
 ...
 • GitHub: ${gh.event.repository.html_url}
  ...
  console.info(changelog.trim() + '\n' + footer)
})

function executeCommand(command) {
  // use GITHUB_WORKSPACE or cwd for git commands (not __dirname which is the action directory)
...
function getCommitsBetween(startSHA, endSHA) {
  ...
  const commits = executeCommand(`git log --pretty=format:"%H:%s" ${startSHA}..${endSHA}`)
