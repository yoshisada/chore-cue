...
test('FR-006: Text input bottom border transitions to gold on focus', async ({
  page,
}) => {
  await page.goto('/login')
  // Step 1: Locate a text input
...
test('FR-017: Chore card hover adds subtle shadow, not harsh drop shadow', async ({
  page,
}) => {
  await page.goto('/')
...
test('FR-010: Reduced motion — transitions complete instantly when prefers-reduced-motion is set', async ({
  page,
}) => {
