...
test('FR-002/US-004: Main screen heading uses italic serif with gold accent for editorial personality', async ({ page }) => {
  await page.goto('/');
...
test('FR-007/US-004: Section labels (Overdue/Due/Upcoming) are uppercase with wide letter spacing', async ({ page }) => {
  await page.goto('/');
...
test('US-004: Metadata text (timestamps, counts) is small, sans-serif, warm grey', async ({ page }) => {
  await page.goto('/');
...
test('FR-002: Font loading — Playfair Display or system serif fallback visible on web', async ({ page }) => {
  await page.goto('/');
...
test('FR-014: Font fallback preserves serif/sans distinction when CDN blocked', async ({ page }) => {
