...
test('FR-011: Dark mode uses inverted palette — dark charcoal background, warm off-white text', async ({
  page,
}) => {
...
test('FR-011: Dark mode preserves serif/sans-serif typographic hierarchy', async ({
  page,
}) => {
  await page.emulateMedia({ colorScheme: 'dark' })
  await page.goto('/')
...
test.skip('spec: Theme toggle in settings switches between light and dark luxury themes — requires credentials', async ({
  page,
}) => {
  // blocked:credentials
  await page.goto('/')
