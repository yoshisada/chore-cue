...
test('FR-012/SC-003: Primary text achieves WCAG AA contrast (4.5:1) against background', async ({ page }) => {
  await page.goto('/');
...
test('FR-012/SC-003: Secondary/metadata text achieves at least 4.5:1 contrast', async ({ page }) => {
  await page.goto('/');
...
test('FR-012/SC-003: Dark mode primary text achieves WCAG AA contrast', async ({ page }) => {
...
test('FR-013/SC-004: Interactive touch targets are at least 48pt (64px) in height', async ({ page }) => {
  await page.goto('/');
...
test('FR-012/SC-003: Navigation header text achieves WCAG AA contrast', async ({ page }) => {
  await page.goto('/');
