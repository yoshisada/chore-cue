...
test('US-005/FR-002: Navigation header uses monochromatic palette and sans-serif type', async ({
  page,
}) => {
  await page.goto('/')
...
test('US-005: Navigation header gold accent only on active/hovered item', async ({
  page,
}) => {
  await page.goto('/')
...
test.skip('US-005: Dialog renders with luxury editorial styling — requires credentials', async ({
  page,
}) => {
  // blocked:credentials
  await page.goto('/')
...
test.skip('spec: Settings page renders with luxury editorial styling — requires credentials', async ({
  page,
}) => {
  // blocked:credentials
...
test.skip('spec: Members page renders with luxury editorial styling — requires credentials', async ({
  page,
}) => {
  // blocked:credentials
  await page.goto('/')
